...
impl<'a> Transaction<'a> {
    pub async fn debit(&mut self, tb: &str, doc: &str, key: &str, rhs: f64) -> Result<Value> {
        if self.ok {
                    ...
                    if let Some(lhs) = k.as_f64() {
                        if lhs - rhs >= 0.0 {
                            if self.cmt {
                                ...
                                self.tx.push(Operation::Debit { tb: tb.into(), doc: doc.into(), key: key.into(), rhs: rhs });
                                return Ok(json!("OK"))
                            }
                        }
                    }
    ...
    pub async fn debit_c(&mut self, tb: &str, doc: &str, key: &str, amt: f64) -> Result<Value> {
        if self.ok {
            return Err(Error::TxFinished);
        }
