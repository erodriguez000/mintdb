...
impl Datastore {
    pub async fn get_key_auth(&self, tb: &str, doc: &str, key: &str) -> Result<Value> {
    ...
    pub async fn get_one_auth(&self, tb: &str, doc: &str) -> Result<Value> {
        let lock = self.collections.try_read().unwrap();
        let res = lock.tables
            .get(tb)
    ...
    pub async fn get_many_auth(&self, tb: &str) -> Result<Value> {
        let mut res = vec![];
        let lock = self.collections.try_read().unwrap();
